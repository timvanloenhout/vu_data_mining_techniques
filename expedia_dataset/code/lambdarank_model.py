...
class RankerNN(object):
    ...
    def evaluate(self, X, y, qid, eval_at=None, valid=False):
        ...
        if valid:
            ...
            df = pd.DataFrame(columns=["srch_id", "pred", "true"], data={"srch_id": qid, "pred": y_pred, "true": y})

            qid = df["srch_id"].unique()
            for ind in qid:
                srch_id_df = df[df["srch_id"]==ind]
                rel_true = [srch_id_df["true"].to_numpy().squeeze()]
                score_pred = [srch_id_df["pred"].to_numpy().squeeze()]

                y_true = np.asarray(rel_true)
                y_pred = np.asarray(score_pred)

                ndcg_list.append(ndcg_score(y_true, y_pred, k=5))

            avg_ndcg = sum(ndcg_list)/len(ndcg_list)
            print("ndcg@5 (valid): {}".format(avg_ndcg))
            return avg_ndcg
    ...
    def _EvalNDCG(self, y, qid, eval_at=None):
        ...
        sumndcg = 0
        count = 0.0
        for qid_unique_idx in tqdm(range(len(qid2idcg))):
            count += 1
